...
export type Entry = {
  rank: number;
  reactiountime: number; // average ms
  time: string; // ISO timestamp
  code: string; // 6-digit
  info: string; // <=32 chars, commas replaced
};
...
export async function readEntries(): Promise<Entry[]> {
  ...
  for (const line of lines) {
    ...
    if (!rankStr || !rtStr || !time || !code) continue;
    const info = infoParts.join(',');
    const rank = Number(rankStr);
    const reactiountime = Number(rtStr);
    if (!Number.isFinite(rank) || !Number.isFinite(reactiountime)) continue;
...
function sanitizeInfo(info: string): string {
...
async function readReserved(): Promise<Set<string>> {
  await ensureFile();
  const txt = await fs.readFile(reservedPath, 'utf-8');
  return new Set(txt.split(/\r?\n/).map(s => s.trim()).filter(Boolean));
}

async function writeReserved(set: Set<string>): Promise<void> {
...
export async function reserveUniqueCode(): Promise<string> {
  ...
  while (used.has(code) || reserved.has(code)) {
    code = randomCode();
    guard++;
    if (guard > 1000) throw new Error('Failed to allocate code');
  }
  reserved.add(code);
...
async function releaseReservedCode(code: string) {
...
export async function insertEntry(reactiountime: number, infoRaw: string | undefined, providedCode?: string) {
  ...
  writeQueue = writeQueue.then(async () => {
    ...
    if (code) {
      ...
      let c = randomCode();
      while (codes.has(c) || reserved.has(c)) c = randomCode();
      code = c;
    }

    const newEntry: Entry = { rank: 0, reactiountime, time, code, info };
